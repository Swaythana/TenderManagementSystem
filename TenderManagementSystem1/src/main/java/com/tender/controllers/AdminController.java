...
public class AdminController {
	...
	@Autowired
	private ConfirmService cserv;
	
	@GetMapping("/")
	public String mainPageLauncher(Model m) {
	...
	@GetMapping("/homee")
	public String homeePageLauncher(Model m) {
	...
	@PostMapping("/conhome1")
	public ModelAndView conhomePageLauncher(@Valid @ModelAttribute("logData") Login log, BindingResult br) {
	...
	@GetMapping("/adminconfirmBiddings")

	public ModelAndView adminBiddingsHandler(){
		
		List<Confirm> result=cserv.getAllBiddings();

		ModelAndView mv = new ModelAndView("confirmBiddingList");

		mv.addObject("confirmBiddingData", result);
